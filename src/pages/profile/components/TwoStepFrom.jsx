...
export default function TwoStepFrom({ next, previous }) {
  ...
  const { handleDataForm, store, setDataStoreFn, initialValuesTwoStep } = useHandleData({ next });
  ...
  const { onChangeSelectorCity } = useCleanOptionsSelector(setCity, store, setDataStoreFn, formRef);

  return (
    <div>
      <Form ref={formRef} onFinish={handleDataForm} initialValues={initialValuesTwoStep}>
        <Form.Item
          label="Municipio"
          name="city"
        ...
        <Form.Item
          ...
          rules={[{ required: true, message: "Campo Requerido" }]}
        >
          <Select options={sectorsOp} onChange={(item) => setDataStoreFn({ sector: item })} />
        </Form.Item>
